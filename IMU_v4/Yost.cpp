...
/**************************************************************************/
Yost::Yost() {
  //const int yostResetPin = 4;

  //digitalWrite(yostResetPin, HIGH);
...
float *Yost::read_accel_filtered() {
  ...
  static float xyz_accel[3]; // store each float (every 4 bytes) into an index of the xyz_euler array
...
float *Yost::rotation_matrix() {
  ...
  byte buffer[36];
  static float rota_matrix[9]; // store each float (every 4 bytes) into an index of the xyz_euler array
  ...
  for(int j = 0; j < 9; j++){
    ...
    // add each float  
    rota_matrix[j] = rotation.rotFloat; // turn byte array to float (union)
  }
 

    
 return &rota_matrix[1];    // 
  }
